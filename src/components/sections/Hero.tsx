    ...
    <section id="home" className={`min-h-screen flex items-center py-16 relative ${colors.bgPrimary}`}>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 z-10">
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center">
          ...
          <div className={`mt-12 lg:mt-0 lg:ml-10 p-8 rounded-lg relative overflow-hidden lg:flex-1 max-w-lg ${colors.bgAccent} bg-opacity-10`}>
            <pre className="font-['Fira_Code',_'Cascadia_Code',_'JetBrains_Mono',_monospace] relative z-10 text-xl lg:text-2xl xl:text-3xl">
              <code className="leading-relaxed">
                <span className={colors.getThemeClass('text-purple-500', 'text-purple-400')}>while</span> <span className={colors.getThemeClass('text-gray-800', 'text-white')}>({colors.getThemeClass('problem', 'problem')} </span><span className={colors.getThemeClass('text-yellow-500', 'text-yellow-400')}>==</span> <span className={colors.getThemeClass('text-green-500', 'text-green-400')}>HARD</span><span className={colors.getThemeClass('text-gray-800', 'text-white')}>) {'{'}</span>
              </code>
              <br />
              <code className="pl-8 leading-relaxed">
                <span className={colors.getThemeClass('text-indigo-500', 'text-indigo-400')}>problem</span> <span className={colors.getThemeClass('text-yellow-500', 'text-yellow-400')}>=</span> <span className={colors.getThemeClass('text-blue-500', 'text-blue-400')}>break</span><span className={colors.getThemeClass('text-gray-800', 'text-white')}>({colors.getThemeClass('problem', 'problem')})</span>
              </code>
              <br />
              <code className="leading-relaxed">
                <span className={colors.getThemeClass('text-gray-800', 'text-white')}>{'}'}</span>
              </code>
            </pre>
          </div>
        </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center">
        ...
        <div className={`w-6 h-10 border-2 rounded-full flex justify-center ${colors.borderPrimary}`}>
          <div className={`w-1 h-3 rounded-full mt-2 animate-bounce ${colors.bgAccent}`}></div>
        </div>
      </div>
    </section>
  );
